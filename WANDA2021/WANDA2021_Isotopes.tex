...
Activation and depletion codes are used to calculate the time dependent fuel isotopic compositions and accumulation of nuclides during and following irradiation. 
Activation and depletion codes generally require input of the energy dependent effective cross sections and particle flux --- both using the same energy discretization --- as well as the duration of the time steps for the calculation. 
From this information total reactions rates are determined and used to solve the system of ordinary differential equations (ODEs) that describe nuclide generation, depletion, and decay. 
This includes the behaviors of the fissile, fertile, burnable poison and transient fission product isotopes, and may include support for time-dependent additions or removals of material from the system.
Several different mathematical methodologies have been used over the decades for determining the solution to the series of ODEs, and the numerical accuracy of these
algorithms are generally sufficient. 
What differentiates activation and depletion codes is in how they generate the total reaction terms which drive the solution and how they interface with nuclear data and transport codes.
% This category of codes is used for the calculation of fuel isotopic compositions as a function of time during reactor operation.
% Using cross section data for the network of multiple interconnected transmutation reactions which occur  in a fissioning system, along with the generation of nuclides through fission yields, these codes track the production and radioactive decay of isotopic species through numerical solution of the Bateman equations.
% This includes the behaviors of the fissile, fertile, burnable poison and transient fission products isotopes, and may include support for time-dependent additions or removals of material from the system.
% The process of tracking the generation or buildup of isotopic species is often referred to as \emph{activation}, and \emph{depletion} is used to denote the burnup of isotopic species due to fission and transmutation.
% In general, all three of these codes overlap in their ability to calculate the evolution of fuel composition during fission, but differ in both the data libraries they draw upon, the incident particles they support, any additional codes that they couple to, as well as additional features unique to each code.
...
ORIGEN is a generalized neutron activation and depletion code packaged as part of the SCALE code suite \cite{Rearden}.
...
HFIRCON is a multi-cycle neutronics and depletion analysis toolkit to automate many irradiation calculations at the High-Flux Isotope Reactor (HFIR).
...
The current version of MCNP6 does include an embedded version of CINDER'90 that can be used for determining criticality conditions.
% k-eigenvalue calculations (a subset of criticality calculations used in time-dependent systems) only.
...
This holds true in the whole fast neutron range (i.e., above neutron resonances for neutron induced reactions). 
For charged-particle induced reactions, the resonance region is practically nonexistent due to the Coulomb barrier, so practically this entire energy range is \enquote{fast},  suitable for calculation using statistical model codes, whose
% Now, if we talk about statistical model codes such as EMPIRE, CoH and essentially TALYS the 
upper energy limit is approximately 200 MeV. 
At higher energies (up to approximately 1 GeV), different reaction models and channels that are not included in  statistical codes (e.g., pion production, intranuclear cascades) are needed.
However, as limited isotope production activities take place in this energy region, the need for GeV-scale predictive codes may be considered a much lower priority than the urgent need for validated codes up to 200 MeV.
...
In considering a possible plan for addressing these data needs in a coordinated fashion, several comments may be made about the most relevant types and energies of data currently needed.
The highest priority are cross sections, since they are usually the easiest to measure. 
Next come ejectile spectra since  the evaporation peak  provides information about level densities, high energies reveal the direct component, and the transition in between is dominated by pre-equilibrium. 
Double-differential (that is, $^{\partial^2\sigma}/_{\partial\Omega\partial E}$, both angle- and energy-differential) particle emission spectra allow one to refine this information. 
This is true for the  emission spectrum of the first ejectile, as subsequent emission spectra should be mostly evaporative, although there will still be a multiple pre-equilibrium component. 
Separating the first emission spectrum from the experimentally observed spectrum will likely require (anti)coincidence detector setups. 
The next priority (and possibly the most useful from a global perspective) would be total cross sections, which give us basic information for constructing the optical model potential.  
Here, we currently have decent coverage (at least for neutrons) already since the total cross section is relatively simple to measure. 
At higher energies, neutrons and protons are very similar after taking into account the Coulomb term in the optical model. 
It is possible to skip neutrons below 20 MeV, since these are again well-characterized to date.
Finally,  simple elastic and inelastic angular distributions  provide essential information for fitting optical model potentials.
Measuring discrete gamma transitions between discrete levels following inelastic scattering is another source of information used in helping fix  model parameters defining spin and parity distribution of levels in the continuum. 
These types of experiments have been  done historically with neutrons below 20 MeV,  but are dubiously feasible at higher energies with charged particles.
It is worth noting that, if a robust predictive capability is needed up to  200 MeV,  these data will need  to be measured to span this entire energy range.
